...
def bin_chi_matrix(chi_mat, chi_bins):
    binned_matrix = chi_mat.clone().detach().long()
    for i, (lower_bound, upper_bound) in enumerate(chi_bins):
        bin_mask = (chi_mat >= lower_bound.float()).__and__(
            chi_mat <= upper_bound.float())
        binned_matrix[bin_mask] = i
