...
def neg_log_prob_to_energy(x, y):
    return -1 * torch.log(y + 1 / 1E20)


def inv_log_prob_to_energy(x, y):
    return 1 / torch.log(y + 1 / 1E20)
...
def get_chi_one_dihedral_constraint(
        constraint: Constraint,
        histogram_dir: str,
        prob_to_energy=neg_log_prob_to_energy) -> str:
    assert type(constraint) == Constraint
    assert constraint.constraint_type == ConstraintType.chi_one_dihedral

    histogram_file = write_histogram_file(
        constraint,
        histogram_dir,
        prob_to_energy=(lambda x, y: 10 * prob_to_energy(x, y)))

    residue_1 = constraint.residue_1

    constraint_line = "Dihedral N {0} CA {0} CB {0} {3} {0} SPLINE chi_one_{0} {1} 0 1 {2}\n".format(
        residue_1.index, histogram_file, constraint.bin_width,
        r2_dict[_aa_1_3_dict[residue_1.identity]])

    return constraint_line


def get_chi_two_dihedral_constraint(
        constraint: Constraint,
        histogram_dir: str,
        prob_to_energy=neg_log_prob_to_energy) -> str:
    assert type(constraint) == Constraint
    assert constraint.constraint_type == ConstraintType.chi_two_dihedral

    histogram_file = write_histogram_file(
        constraint,
        histogram_dir,
        prob_to_energy=(lambda x, y: 8 * prob_to_energy(x, y)))

    residue_1 = constraint.residue_1

    constraint_line = "Dihedral CA {0} CB {0} {3} {0} {4} {0} SPLINE chi_two_{0} {1} 0 1 {2}\n".format(
        residue_1.index, histogram_file, constraint.bin_width,
        r2_dict[_aa_1_3_dict[residue_1.identity]],
        r3_dict[_aa_1_3_dict[residue_1.identity]])

    return constraint_line


def get_chi_three_dihedral_constraint(
        constraint: Constraint,
        histogram_dir: str,
        prob_to_energy=neg_log_prob_to_energy) -> str:
    assert type(constraint) == Constraint
    assert constraint.constraint_type == ConstraintType.chi_three_dihedral

    histogram_file = write_histogram_file(
        constraint,
        histogram_dir,
        prob_to_energy=(lambda x, y: 6 * prob_to_energy(x, y)))

    residue_1 = constraint.residue_1

    constraint_line = "Dihedral CB {0} {3} {0} {4} {0} {5} {0} SPLINE chi_three_{0} {1} 0 1 {2}\n".format(
        residue_1.index, histogram_file, constraint.bin_width,
        r2_dict[_aa_1_3_dict[residue_1.identity]],
        r3_dict[_aa_1_3_dict[residue_1.identity]],
        r4_dict[_aa_1_3_dict[residue_1.identity]])

    return constraint_line


def get_chi_four_dihedral_constraint(
        constraint: Constraint,
        histogram_dir: str,
        prob_to_energy=neg_log_prob_to_energy) -> str:
    assert type(constraint) == Constraint
    assert constraint.constraint_type == ConstraintType.chi_four_dihedral

    histogram_file = write_histogram_file(
        constraint,
        histogram_dir,
        prob_to_energy=(lambda x, y: 4 * prob_to_energy(x, y)))

    residue_1 = constraint.residue_1

    constraint_line = "Dihedral {3} {0} {4} {0} {5} {0} {6} {0} SPLINE chi_four_{0} {1} 0 1 {2}\n".format(
        residue_1.index, histogram_file, constraint.bin_width,
        r2_dict[_aa_1_3_dict[residue_1.identity]],
        r3_dict[_aa_1_3_dict[residue_1.identity]],
        r4_dict[_aa_1_3_dict[residue_1.identity]],
        r5_dict[_aa_1_3_dict[residue_1.identity]])

    return constraint_line


def get_chi_five_dihedral_constraint(
        constraint: Constraint,
        histogram_dir: str,
        prob_to_energy=neg_log_prob_to_energy) -> str:
    assert type(constraint) == Constraint
    assert constraint.constraint_type == ConstraintType.chi_five_dihedral

    histogram_file = write_histogram_file(
        constraint,
        histogram_dir,
        prob_to_energy=(lambda x, y: 2 * prob_to_energy(x, y)))

    residue_1 = constraint.residue_1

    constraint_line = "Dihedral {3} {0} {4} {0} {5} {0} {6} {0} SPLINE chi_five_{0} {1} 0 1 {2}\n".format(
        residue_1.index, histogram_file, constraint.bin_width,
        r3_dict[_aa_1_3_dict[residue_1.identity]],
        r4_dict[_aa_1_3_dict[residue_1.identity]],
        r5_dict[_aa_1_3_dict[residue_1.identity]],
        r6_dict[_aa_1_3_dict[residue_1.identity]])
...
constraint_type_generator_dict = {
    ...
    ConstraintType.chi_one_dihedral: get_chi_one_dihedral_constraint,
    ConstraintType.chi_two_dihedral: get_chi_two_dihedral_constraint,
    ConstraintType.chi_three_dihedral: get_chi_three_dihedral_constraint,
    ConstraintType.chi_four_dihedral: get_chi_four_dihedral_constraint,
    ConstraintType.chi_five_dihedral: get_chi_five_dihedral_constraint
}
