...
def truncate_fasta_for_pdb(fasta_file, pdb_file, warn_pdbs, pdb_id=""):
    ...
    for record in SeqIO.parse(fasta_file, "fasta"):
        fasta_sequences.append(record.seq._data)
    ...
    if pdb_h_seq == None or pdb_l_seq == None or len(pdb_h_seq) == 0 or len(
        ...
        print("Missing {} chain seq for PDB: {}".format(
            "H" if len(pdb_h_seq) == 0 else "L", pdb_id))
        warn_pdbs.append(pdb_id)
        return
...
def _cli():
    ...
    parser.add_argument('--use_existing_summary',
                        default=False,
                        action="store_true",
                        help='Download summary file (false to use existing)')
    args = parser.parse_args()

    download_train_dataset(
        download_summary_file=(not args.use_existing_summary),
        seqid=args.seqid,
        paired=args.paired,
