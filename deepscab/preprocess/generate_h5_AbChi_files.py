...
def antibody_to_h5(pdb_dir,
    ...
    l3_set = h5_out.create_dataset('l3_range', (num_seqs, 2),
                                   ...
                                   dtype='uint16',
                                   fillvalue=-1)
    dist_angle_set = h5_out.create_dataset(
        'dist_angle_mat', (num_seqs, 4, max_total_len, max_total_len),
        maxshape=(None, 4, max_total_len, max_total_len),
        compression='lzf',
        dtype='float',
        fillvalue=-1)
    chi_one_set = h5_out.create_dataset('chi_one_mat',
                                        (num_seqs, 1, max_total_len),
                                        maxshape=(None, 1, max_total_len),
                                        compression='lzf',
                                        dtype='float',
                                        fillvalue=-1)

    chi_two_set = h5_out.create_dataset('chi_two_mat',
                                        (num_seqs, 1, max_total_len),
                                        maxshape=(None, 1, max_total_len),
                                        compression='lzf',
                                        dtype='float',
    ...
    chi_three_set = h5_out.create_dataset('chi_three_mat',
                                          (num_seqs, 1, max_total_len),
                                          maxshape=(None, 1, max_total_len),
                                          compression='lzf',
                                          dtype='float',
    ...
    chi_four_set = h5_out.create_dataset('chi_four_mat',
                                         (num_seqs, 1, max_total_len),
                                         maxshape=(None, 1, max_total_len),
                                         compression='lzf',
                                         dtype='float',
    ...
    chi_five_set = h5_out.create_dataset('chi_five_mat',
                                         (num_seqs, 1, max_total_len),
                                         maxshape=(None, 1, max_total_len),
                                         compression='lzf',
                                         dtype='float',
    ...
    for index, file in tqdm(enumerate(pdb_files),
        ...
        try:
            dist_angle_set[index, :4, :total_len, :total_len] = np.array(
                info['dist_angle_mat'])
        except TypeError:
            msg = ('Fasta/PDB coordinate length mismatch: the fasta sequence '
                   ...
                   'mismatch.\n ')
            raise ValueError(
                msg.format(total_len, len(info['dist_angle_mat']), pdb_file))

        try:
            chi_one_set[index, :1, :total_len] = np.array(info['chi_one_mat'])
            chi_two_set[index, :1, :total_len] = np.array(info['chi_two_mat'])
            chi_three_set[index, :1, :total_len] = np.array(
                info['chi_three_mat'])
            chi_four_set[index, :1, :total_len] = np.array(
                info['chi_four_mat'])
            chi_five_set[index, :1, :total_len] = np.array(
                info['chi_five_mat'])
            ...
            msg = ('Fasta/PDB coordinate length mismatch: the fasta sequence '
                   ...
                   'mismatch.\n ')
            raise ValueError(
                msg.format(total_len, len(info['chi_one_mat']), pdb_file))
